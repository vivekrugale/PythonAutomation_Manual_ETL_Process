...
with open('/iics_pmroot/Temp/smpl-list.txt', 'w') as f:
    f.write((df_report['Country Code'].dropna() + ';' + df_report['Call Sample: ID'].dropna()).to_string(header=False, index=False))

#Unix command to run shell script to search zp filenames.
grepFileSearch = """
/iics_pmroot/Scripts/chk-smpl-list-new.sh /iics_pmroot/Temp/smpl-list.txt > /iics_pmroot/Temp/zpUnixOutput.txt
"""
#This command will take some time to execute.
unix(grepFileSearch)

df_unixOutput = pd.read_csv('/iics_pmroot/Temp/zpUnixOutput.txt', header=None)
df_unixOutput.rename(columns={0:'temp'},inplace=True)

df_sampleList = pd.DataFrame(columns=['Call Sample: ID','ZP File Name'])
df_sampleList[['Call Sample: ID','ZP File Name']] = df_unixOutput['temp'].str.split(':',expand=True)

#Merge to df on ID. Creates a new column at the end.
...
#For files not found.
if 'nf' in finalReport['ZP File Name'].values:
    nf_count = finalReport['ZP File Name'].value_counts()['nf']
    df_nfList = df_sampleList[df_sampleList['ZP File Name'] == 'nf']
    ...
    <body>
        ...
        <p></p>
        <p>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;For the ZP pending sample report of {todaysDate}, there are {sampleCount} call sample IDs found for which no files are present.
        </p>
        <p></p>
        ...
        <p></p>
        <p></p>
        <p style="margin-top: 30px;">Thanks and regards,<br>Vivek Rugale</p>
    </body>
    </html>
    """
    ...
    subject = f"Files not found for pending samples report on {todaysDate}"
